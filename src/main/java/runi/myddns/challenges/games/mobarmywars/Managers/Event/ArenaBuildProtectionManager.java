...
public class ArenaBuildProtectionManager implements Listener {
    ...
    public ArenaBuildProtectionManager(MobArmyWarsGame game) {
        this.game = game;
    ...
    public void loadSpawnProtectionAreas() {
        ...
        if (!file.exists()) {
            game.getPlugin().getLogger().warning("[MobArmyWars] ⚠ spawns.yml nicht gefunden – Lobby-Schutzbereiche nicht geladen!");
            return;
        }
    ...
    private void loadWaveArea(FileConfiguration cfg, String path) {
        ...
        if (world == null) {
            game.getPlugin().getLogger().warning("[MobArmyWars] ⚠ Welt '" + worldName + "' für " + path + " nicht gefunden!");
            return;
        }
        ...
        if (c1 == null || c2 == null) {
            game.getPlugin().getLogger().warning("[MobArmyWars] ⚠ Ungültige Corner für " + path);
            return;
        }
