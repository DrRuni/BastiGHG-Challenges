...
public class WorldManager {
    ...
    public WorldManager(MobArmyWarsGame game) {
        this.game = game;
    }
    ...
    public void unloadWorlds() {
        unloadWorld(WORLD_LOBBY);
        unloadWorld(WORLD_ARENA);
        unloadWorld("world_rot");
        unloadWorld("world_blau");
        unloadWorld("world_rot_nether");
        unloadWorld("world_blau_nether");
    }
    ...
    private void unloadWorld(String name) {
        ...
        if (!Bukkit.unloadWorld(world, true)) {
            game.getPlugin().getLogger().warning("Welt konnte nicht entladen werden: " + name);
        }
    }
    ...
    private void extractTemplateWorldFromZip(String worldName) {
        ...
        if (zipFile == null || !zipFile.exists()) {
            game.getPlugin().getLogger().severe("❌ Keine Template-ZIP gefunden: world_mobarmy V*.zip");
            return;
        }
        ...
        try (ZipInputStream zis = new ZipInputStream(new FileInputStream(zipFile))) {
            ...
            game.getPlugin().getLogger().severe("❌ Fehler beim Entpacken der Welt " + worldName + ": " + e.getMessage());
            return;
        }

        if (!found) {
            game.getPlugin().getLogger().severe("❌ Welt " + worldName + " wurde in " + zipFile.getName() + " nicht gefunden!");
        }
    }
    ...
    private void deleteDirectoryRecursively(File dir) {
        ...
        if (files != null) {
            for (File f : files) {
                if (f.isDirectory()) {
                    ...
                    if (!f.delete()) {
                        game.getPlugin().getLogger().warning("⚠ Konnte Datei nicht löschen: " + f.getPath());
                    }
                }
            }
        }
        ...
        if (!dir.delete()) {
            game.getPlugin().getLogger().warning("⚠ Konnte Ordner nicht löschen: " + dir.getPath());
        }
    }
    ...
    private File findNewestTemplateZipInDataFolder() {
        ...
        File[] files =
                mobArmyFolder.listFiles((_, name) ->
