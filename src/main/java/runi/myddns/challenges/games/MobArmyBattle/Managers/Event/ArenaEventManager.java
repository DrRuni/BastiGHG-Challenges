...
import runi.myddns.challenges.games.MobArmyBattle.Managers.World.ResumeManager;
import runi.myddns.challenges.games.MobArmyBattle.Managers.World.TeleportManager;
...
public class ArenaEventManager implements Listener {
    ...
    public ArenaEventManager(MobArmyBattleGame game) {
        this.game = game;
        this.scoreboardManager = new ArenaScoreboardManager(game, this);
    ...
    public void teleportTeamsToArena() {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (!game.getTeamManager().isInTeam(player)) continue;

            game.getEventResume().savePlayerSpawn(player, player.getLocation());

            TeleportManager.teleportToArena(game, player);
        ...
        game.getEventResume().savePhase(ResumeManager.PHASE_ARENA);
    }
    ...
    public void startArenaEvent() {
        ...
        int teleportTaskId = Bukkit.getScheduler()
                .runTaskLater(
                        game.getPlugin(),
                        () -> {
        ...
        int countdownTaskId = Bukkit.getScheduler().runTaskLater(game.getPlugin(), () -> {
    ...
    public void startWaveBattle() {
        ...
        if (waveManager == null) {
            game.getPlugin().getLogger().warning("[ArenaManager] ❌ WaveManager ist null! Abbruch von startWaveBattle().");
            return;
        }

        ArenaConfig.ArenaData arena = game.getArenaConfig().getActiveArena();

        if (arena == null) {
            game.getPlugin().getLogger().warning("[ArenaManager] ❌ Keine aktive Arena geladen! Wave abgebrochen.");
            return;
        }
        ...
        if (arenaWorld == null) {
            game.getPlugin().getLogger().warning("[ArenaManager] ❌ Arena-Welt nicht geladen: " + arena.world());
            return;
        }
    ...
    private void spawnWave(String teamFighting, String waveOwner, Location[] spawnPoints) {
        ...
        if (!waveRunning.getOrDefault(waveOwner, false) && !finishedTeams.contains(waveOwner)) {
            game.getPlugin().getLogger().info("⏹ Wave-Spawning abgebrochen: waveOwner=" + waveOwner + " ist nicht aktiv.");
            return;
        }
    ...
    private void spawnWaveMobs(String teamFighting, String waveOwner, Location[] spawnPoints,
        ...
        for (WaveManager.WaveEntry entry : wave) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                }.runTaskLater(game.getPlugin(), finalDelay);
    ...
    private void monitorWaveProgress(String teamFighting, String waveOwner, Location[] spawnPoints,
        ...
        }.runTaskTimer(game.getPlugin(), delay + 20L, 40L).getTaskId();
        game.getArenaManager().registerTask(taskId);
    }
    ...
    private void handleEmptyWave(String teamFighting, String waveOwner, Location[] spawnPoints, int finalWave) {
        ...
        int emptyWaveTaskId = Bukkit.getScheduler().runTaskLater(game.getPlugin(), () -> {
    ...
    private void checkWinner() {
        ...
        game.getPlugin().getLogger().info("🏁 Arena beendet. Gewinner: " + winningTeam);
    ...
    public void markPlayerReady(Player player) {
        ...
        if (game.getEventResume().loadPhase()
        ...
        for (Player onlinePlayer : Bukkit.getOnlinePlayers()) {
            ...
            String onlineTeam =
                    game.getTeamManager().getPlayerTeam(onlinePlayer);
    ...
    public void resetArena() {
        ...
        for (Player p : Bukkit.getOnlinePlayers()) {
            ...
            if (game.getTimerManager() != null) {
                game.getTimerManager().removeBossBarFor(p);
            }

            game.getArenaBuildProtectionManager().clearTeamData("rot");
            game.getArenaBuildProtectionManager().clearTeamData("blau");
        }
    ...
    private void startAggroTask() {
                ...
                for (List<LivingEntity> mobs : activeMobs.values()) {
                    for (LivingEntity mob : mobs) {
                        ...
                        if (aiMob.getTarget() instanceof Player currentTarget) {
                            String team = game.getTeamManager().getPlayerTeam(currentTarget);
        ...
        }.runTaskTimer(game.getPlugin(), 0L, 20L).getTaskId();
    }
    ...
    private Component getTeamDisplayName(String team) {
        ...
        if (team.equalsIgnoreCase("Rot")) {
            return game.getLanguageManager().getComponent(
