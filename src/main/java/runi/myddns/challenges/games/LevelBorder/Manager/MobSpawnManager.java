...
public class MobSpawnManager {
    ...
    public MobSpawnManager(LevelBorderGame game, BorderDataManager data, LevelBorderManager borderManager) {
        this.game = game;
        this.data = data;
    ...
    public void start() {
        ...
        if (!game.getConfig().getBoolean("mob-spawning.enabled", true)) {
            game.getPlugin().getLogger().info("LevelBorder MobSpawning ist deaktiviert.");
            return;
        }

        long interval = game.getPlugin().getConfig().getLong("mob-spawning.interval-ticks", 20L);
        ...
        game.getPlugin().getLogger().info("LevelBorder MobSpawning gestartet. Intervall: " + interval + " Ticks");
    }
    ...
    private void runSpawnTick() {
        if (!game.getPlugin().getConfig().getBoolean("mob-spawning.enabled", true)) {
        ...
        int perWorldCap = game.getPlugin().getConfig().getInt("mob-spawning.caps.per-world", 80);
        ...
        int rounds = game.getPlugin().getConfig().getInt("mob-spawning.attempts.rounds-per-check", 1);
    ...
    private void trySpawnNearPlayer(Player player) {
        ...
        int perPlayerCap = game.getPlugin().getConfig().getInt("mob-spawning.caps.per-player", 8);
        ...
        int candidates = game.getPlugin().getConfig().getInt("mob-spawning.attempts.candidate-positions", 5);
    ...
    private Location randomCandidateAround(Player player) {
        ...
        int maxDistance = game.getPlugin().getConfig().getInt("mob-spawning.distance.max-to-player", 96);

        double borderSize = data.getSize();
        double assistInsideUntil = game.getPlugin().getConfig().getDouble(
        ...
        double insideChance = game.getPlugin().getConfig().getDouble(
        ...
        double surfaceChance = game.getPlugin().getConfig().getDouble(
        ...
        int searchDown = game.getPlugin().getConfig().getInt(
    ...
    private boolean isFarEnoughFromAllPlayers(Location loc) {
        int minDistance = game.getPlugin().getConfig().getInt("mob-spawning.distance.min-to-player", 28);
        double minSquared = minDistance * minDistance;

        for (Player player : loc.getWorld().getPlayers()) {
            if (!game.isLevelBorderPlayer(player)) continue;
    ...
    private boolean isChunkCapReached(Location loc) {
        int cap = game.getPlugin().getConfig().getInt("mob-spawning.caps.per-chunk", 3);
    ...
    private void addMobOption(List<MobOption> options, String pathName, EntityType type) {
        ...
        if (!game.getPlugin().getConfig().getBoolean(path + ".enabled", true)) return;

        int weight = game.getPlugin().getConfig().getInt(path + ".weight", 100);
        int maxPackSize = game.getPlugin().getConfig().getInt(path + ".max-pack-size", 3);
    ...
    private int spawnPack(EntityType type, Location center, int mobMaxPackSize) {
        ...
        int packSearchDown = game.getPlugin().getConfig().getInt(
        ...
        for (int i = 0; i < maxPackSize; i++) {
            ...
            entity.setMetadata(META_KEY, new org.bukkit.metadata.FixedMetadataValue(game.getPlugin(), true));
    ...
    private boolean debugPluginSpawns() {
        return game.getPlugin().getConfig().getBoolean("mob-spawning.debug.plugin-spawns", false);
    }

    private boolean debugFailedAttempts() {
        return game.getPlugin().getConfig().getBoolean("mob-spawning.debug.failed-attempts", false);
    }

    private void debug(String message) {
        if (!debugFailedAttempts()) return;
        game.getPlugin().getLogger().info("[MobSpawnDebug] " + message);
    }
}
