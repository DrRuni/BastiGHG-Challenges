...
public class LevelBorderManager {
    ...
    public LevelBorderManager(LevelBorderGame game, BorderDataManager data, ScoreboardManager scoreboard) {
        this.game = game;
        this.data = data;
    ...
    public void resetBorder(Player initiator) {
        game.getPlugin().getLogger().info(
                ConsoleColor.RED + "     🔄 LevelBorder-Reset wurde von "
                        + initiator.getName()
                        + " ausgeführt."
                        + ConsoleColor.RESET
        );
    ...
    private void applyBorderToWorld(World world, double size, long transitionSeconds) {
        ...
        border.setCenter(
                Math.floor(center.getX()) + 0.5,
                Math.floor(center.getZ()) + 0.5
        );
    ...
    private void applyBorderToAllWorlds(double size, long transitionSeconds) {
        ...
        applyBorderToWorld(Bukkit.getWorld(overworld.getName() + "_nether"), size, transitionSeconds);
        applyBorderToWorld(Bukkit.getWorld(overworld.getName() + "_the_end"), size, transitionSeconds);
    }
    ...
    public BorderDataManager getData() {
        return data;
    }

    public LevelBorderGame getGame() {
        return game;
    }
}